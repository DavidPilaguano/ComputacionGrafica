...
{
    public partial class FrmBSplineCoxDeBoor : Form
    {
        ...
        private const int PUNTOS_MINIMOS = GRADO_CURVA + 1; 

        public FrmBSplineCoxDeBoor()
        {
            InitializeComponent();
            this.Text = $"B-Spline Cox-De Boor Recursivo (Grado {GRADO_CURVA})";
        ...
        private void btnDibujar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // 2. Generar la curva B-Spline usando la lógica del Modelo
                _puntosCurva = B_Spline_CoxDeBoor.GenerarCurva(_puntosControl, GRADO_CURVA);

                // 3. Forzar el redibujo
                pnlGrafico.Invalidate();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al dibujar la curva: " + ex.Message, "Error de B-Spline",
