...
{
    // Implementación usando la Matriz Base de B-Spline Cúbica Uniforme
    public partial class FrmBSplineMatriz : Form
    {
        ...
        private const int GRADO_CURVA = 3; 
        private const int PUNTOS_MINIMOS = 4; 

         public FrmBSplineMatriz()
        {
            InitializeComponent();
            this.Text = $"B-Spline Matricial Uniforme (Grado {GRADO_CURVA})";
        ...
        private void btnDibujar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception($"Para una B-Spline Matricial Cúbica se requieren al menos {PUNTOS_MINIMOS} puntos de control.");
                }

                _puntosCurva = B_Spline_Matriz.GenerarCurva(_puntosControl);
               
                pnlGrafico.Invalidate();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al dibujar la curva: " + ex.Message, "Error de B-Spline (Matriz)",
